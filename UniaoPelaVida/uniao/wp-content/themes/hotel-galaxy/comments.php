...
<?php if ( comments_open() ) : ?>
	<div class="comment-form animate" data-anim-type="fadeInUp" data-anim-delay="800">
		...
		$defaults = array(
			...
			'comment_field'=> '<textarea id="comment" name="comment" placeholder="Deixe seu comentário" rows="6"></textarea>',
			'class_form'      => 'form-inline',		
			'logged_in_as' => '<p class="logged-in-as">' . __( "
Logado como ",'hotel-galaxy' ).'<a href="'. admin_url( 'profile.php' ).'">'.$user_identity.'</a>'. '<a href="'.esc_url(wp_logout_url( get_permalink() )) .'" title="'.esc_attr('Deseja sair de sua conta?').'">'.__(" - Sair",'hotel-galaxy').'</a>' . '</p>',
			'title_reply_to' => __( 'Leave a Reply to %s','hotel-galaxy'),
			'class_submit' => 'custom-btn book-lg',
			'label_submit'=>__( 'Enviar','hotel-galaxy'),
			'comment_notes_before'=> '',
			'comment_notes_after'=>'',
			'title_reply'=> '<h2>'.__('Deixe um comentário','hotel-galaxy').'</h2><p>Seu endereço de e-mail não será publicado. Campos obrigatórios são marcados*</p>',		
			'role_form'=> 'form',		
			);
